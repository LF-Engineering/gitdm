...
docker run -p 17070:7070 -e "GITDM_GIT_REPO=${GITDM_GIT_REPO}" -e "GITDM_GIT_USER=${GITDM_GIT_USER}" -e "GITDM_GIT_OAUTH=${GITDM_GIT_OAUTH}" -e "DA_API_URL=${DA_API_URL}" -it "${DOCKER_USER}/lf-gitdm-sync" "/bin/sh"
