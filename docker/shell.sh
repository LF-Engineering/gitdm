...
docker run -p 17070:7070 -e "DA_API_URL=${DA_API_URL}" -e "GITDM_GITHUB_REPO=${GITDM_GITHUB_REPO}" -e "GITDM_GITHUB_USER=${GITDM_GITHUB_USER}" -e "GITDM_GITHUB_OAUTH=${GITDM_GITHUB_OAUTH}" -e "GITDM_GIT_USER=${GITDM_GIT_USER}" -e "GITDM_GIT_EMAIL=${GITDM_GIT_EMAIL}" -it "${DOCKER_USER}/lf-gitdm-sync" "/bin/sh"
