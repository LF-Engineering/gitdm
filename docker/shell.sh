...
docker run -p 17070:7070 -e "GITDM_GITHUB_REPO=${GITDM_GITHUB_REPO}" -e "GITDM_GITHUB_USER=${GITDM_GITHUB_USER}" -e "GITDM_GITHUB_OAUTH=${GITDM_GITHUB_OAUTH}" -e "DA_API_URL=${DA_API_URL}" -it "${DOCKER_USER}/lf-gitdm-sync" "/bin/sh"
